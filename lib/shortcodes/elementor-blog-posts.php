...
function elementor_blog_posts() {
	...
	$article_count = count( $categories ) * 6;
  
	?>
<div class="Elementor__Blog">
  <ul class="Elementor__Blog--Categories flex flex-justify-center gap-2">
	<?php
	foreach ( $categories as $category ) {
		if ( $category->count > 1 ) {
			?>
	<li data-id="<?= esc_attr( 'category-' . $category->slug ); ?>">
	<input class="filter-item" id="<?= esc_attr( 'category-' . $category->slug ); ?>" type="radio" value="<?= esc_attr( $category->slug ); ?>" name="category" />
